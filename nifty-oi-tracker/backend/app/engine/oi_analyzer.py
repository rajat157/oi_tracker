...
Analyzes option chain OI data to determine market sentiment using a
zone-separated tug-of-war model with conviction weighting, momentum
integration, and hysteresis-based verdict determination.

Ported from the original oi_analyzer.py -- all pure functions, no DB/API deps.
"""
...
# NIFTY lot size (not in constants because it changes per exchange circular)
NIFTY_LOT_SIZE = 65
...
def calculate_market_trend(
    ...
    regime_counts: dict[str, int] = {"trending_up": 0, "trending_down": 0, "range_bound": 0}
    for entry in recent:
    ...
    majority_regime = max(regime_counts, key=regime_counts.get)  # type: ignore[arg-type]
...
def calculate_dynamic_sl_pct(
    ...
    # IV-based SL percentage using tier table from constants
    for iv_threshold, sl_pct in IV_SL_TIERS:
        if iv < iv_threshold:
            return sl_pct / 100.0

    # Fallback (should not reach here given inf in tiers)
    return 0.25  # High IV = wider SL (max 25%)


# ---------------------------------------------------------------------------
# OI acceleration & premium momentum
# ---------------------------------------------------------------------------

def calculate_oi_acceleration(
    prev_oi_changes: list[tuple],
    current_call_change: float,
    current_put_change: float,
) -> dict:
    """
...
def calculate_premium_momentum(
    current_strikes: dict,
    prev_strikes: dict | None,
    spot_price: float,
) -> dict:
    """
...
def calculate_trade_setup(
    ...
    strike_to_buy: int | None = None
    premium = 0.0
    strike_moneyness: str | None = None
    ...
    if is_bullish:
        ...
        for strike_candidate, moneyness in candidates:
            if strike_candidate and strikes_data.get(strike_candidate, {}).get("ce_ltp", 0) > 0:
                strike_to_buy = strike_candidate
                premium = strikes_data[strike_candidate]["ce_ltp"]
        ...
        candidates = [(itm_strike, "ITM"), (atm_strike, "ATM"), (otm_strike, "OTM")]
        for strike_candidate, moneyness in candidates:
            if strike_candidate and strikes_data.get(strike_candidate, {}).get("pe_ltp", 0) > 0:
                strike_to_buy = strike_candidate
                premium = strikes_data[strike_candidate]["pe_ltp"]
                strike_moneyness = moneyness
                break
    ...
    # Calculate IV-based dynamic SL percentage (15-25% range)
    ...
    # Calculate rupee risk/reward (NIFTY lot size)
...
# ---------------------------------------------------------------------------
# Signal confidence & trap detection
# ---------------------------------------------------------------------------
...
def analyze_tug_of_war(
    strikes_data: dict,
    spot_price: float,
    num_strikes: int = NUM_OTM_STRIKES,
    momentum_score: float | None = None,
    price_history: list[dict] | None = None,
    vix: float = 0.0,
    futures_oi_change: float = 0.0,
    prev_oi_changes: list[tuple] | None = None,
    prev_strikes_data: dict | None = None,
    total_oi_weight: float = TOTAL_OI_WEIGHT,
    prev_verdict: str | None = None,
) -> dict:
    """
    ...
    - Force = (0.85 x OI Change) + (0.15 x Total OI / scale_factor)
    ...
    all_oi_changes: list[float] = []
    all_total_oi: list[float] = []
    ...
    def calculate_force(oi_change: float, total_oi: float, conviction: float) -> float:
        """Calculate force = conviction x ((1-w) x OI Change + w x Total OI / scale)"""
    ...
    # ========================================
    otm_puts_data: list[dict] = []
    otm_puts_total_oi = 0
    otm_puts_total_oi_change = 0
    otm_puts_total_force = 0.0
    ...
    # ========================================
    itm_calls_data: list[dict] = []
    itm_calls_total_oi = 0
    itm_calls_total_oi_change = 0
    itm_calls_total_force = 0.0
    ...
    # ========================================
    otm_calls_data: list[dict] = []
    otm_calls_total_oi = 0
    otm_calls_total_oi_change = 0
    otm_calls_total_force = 0.0
    ...
    # ========================================
    itm_puts_data: list[dict] = []
    itm_puts_total_oi = 0
    itm_puts_total_oi_change = 0
    itm_puts_total_force = 0.0
    ...
    # ========================================
    below_spot_data: list[dict] = []
    total_below_bullish = 0.0
    total_below_bearish = 0.0
    ...
    above_spot_data: list[dict] = []
    total_above_bullish = 0.0
    total_above_bearish = 0.0
    ...
    # Volume PCR
    total_volume = total_below_volume + total_above_volume  # noqa: F841
